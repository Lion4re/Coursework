...
class LinearClassifier(object):
  ...
  def train_linear_classifier(self, X_train, y_train, X_val, y_val, learning_rate=1e-3, reg=1e-5, reg_type = 2, num_epochs = 6, batch_size = 200, num_valid_loss_evals = 100, verbose=False):
    ...
    for it in range(num_iters):
      ...
      loss, gradient = self.loss(X_batch, y_batch, reg, reg_type, 0)
      train_loss = loss
      ...
      if verbose and it % 100 == 0:
        ...
        print(f'iteration No {it} / {num_iters}: loss {train_loss}')


      if verbose and it % num_valid_loss_evals == 0:
        valid_loss, _ = self.loss(X_val, y_val, reg, reg_type, 1)
        valid_loss_history.append(valid_loss)
        print(f'iteration No {it} / {num_iters}: validation loss {valid_loss}')
