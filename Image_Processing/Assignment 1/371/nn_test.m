...
for i = 1:size(images,1)%size(images,1)
    ...
    %nearest neighbor function too.
    sub_image = my_imresize_NN(img, 8, "sub");
    restored = my_imresize_NN(sub_image, 8, "up");
...
%     figure();
%     imshow(restored), title("my_imresize_nn Restored Image");
%     figure();
